...
def index(request, course_id, test_task_id):
    ...
    # Следующий тест
    next_test = Test_task.objects.filter(course = course_id)[int(test_task_id):int(test_task_id)+2]
    # Вопросы к тесту
        ...
        if request.method == "POST" and request.is_ajax():
            ...
            if next_test:
                context = {
                    ...
                    'grade':grade, 
                    'course_id': course_id, 
                    'next_test': int(test_task_id)+1,
                }
            else:
                context = {'test_question_list' : test_question_list, 'answer_list' : answer_list, 'user_answers':user_answers, 'grade':grade }
